import {
  ...
  let initialState_tokenPrice = new UInt64(2);  // token / MINA
  ...
  export class FungibleToken extends TokenContract {
    ...
    async initialize(
      ...
      // permissions.receive = Permissions.none(), // 允许接收@@@@@@@@@@@@@@@@
    ...
    @method
    async buy(sender: PublicKey, amountMINA: UInt64) {
      ...
      senderUpdate.send({ to: owner, amount: amountMINA });

      // fetch onchain states
      let targetAmount = this.targetAmount.getAndRequireEquals();
      let totalBalanceOfToken = await this.getBalanceOf(this.address);
      // culculate mint token amount
      let tokenPrice = new UInt64(2);
      let tokenAmount = tokenPrice.mul(amountMINA);

      tokenAmount.add(totalBalanceOfToken).assertLessThanOrEqual(targetAmount);

      // mint token to sender
      await this.mint(sender, tokenAmount);
    }
