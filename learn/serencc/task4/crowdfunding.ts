...
export class Crowdfunding extends SmartContract {
  ...
  @method async withdraw(owner: PublicKey) {
    ...
    const currentTime = this.network.blockchainLength.getAndRequireEquals();
    const endTime = this.endTime.getAndRequireEquals();
    currentTime.assertGreaterThanOrEqual(endTime, 'Funding period is not over');

    const withdrawal = this.account.balance.getAndRequireEquals();

    const senderUpdate = AccountUpdate.createSigned(currentOwner);
    this.send({ to: senderUpdate, amount: withdrawal });

    senderUpdate.account.timing.set({
      initialMinimumBalance: withdrawal,
      cliffTime: UInt32.from(0),
      cliffAmount: withdrawal.div(10).mul(2),
      vestingPeriod: UInt32.from(200),
      vestingIncrement: withdrawal.div(10),
    });
  }
}
