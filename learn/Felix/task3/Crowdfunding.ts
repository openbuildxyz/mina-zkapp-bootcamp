...
退款功能不限制任何人调用, 因为用publicKey验证, 向publicKey转账, 与发送者无关, Dapp可以代为执行

只需要存储好验证哈希的:num, funderPublicKey,amount,oldHash.
通过事件获取

尝试不使用Merkle Tree实现众筹失败后退款
因为链上状态有限, 设计了一个方法,回退哈希值,思路类似递归.
退款函数:重新计算哈希并验证,fundHash回退为oldHash
