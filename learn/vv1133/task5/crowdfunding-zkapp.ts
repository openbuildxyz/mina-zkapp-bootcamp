...
export class CrowdfundingZkapp extends SmartContract {
  ...
  async deposit(amount: UInt64) {
    ...
    senderUpdate.send({ to: this.address, amount: validAmount });
    senderUpdate.body.mayUseToken = AccountUpdate.MayUseToken.InheritFromParent; // MUST ADD THIS!
  }
  ...
  async withdraw() {
    ...
    const recieverUpdate = this.send({ to: withdrawer, amount: currentBal });
    recieverUpdate.body.mayUseToken =
