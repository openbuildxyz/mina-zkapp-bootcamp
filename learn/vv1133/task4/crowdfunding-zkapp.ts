...
export class CrowdfundingZkapp extends SmartContract {
  ...
  async withdraw() {
    ...
    const recieverAcctUpt = AccountUpdate.createSigned(withdrawer);
    //recieverAcctUpt.account.isNew.requireEquals(Bool(true));
    this.send({ to: recieverAcctUpt, amount: currentBal });

    // !!!vesting schedule!!!
    recieverAcctUpt.account.timing.set({
      initialMinimumBalance: currentBal,
      cliffTime: new UInt32(0),
      cliffAmount: currentBal.mul(2).div(10),
      vestingPeriod: new UInt32(200),
      vestingIncrement: currentBal.div(10),
    });
  }
}
