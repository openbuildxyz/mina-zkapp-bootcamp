...
import { MkingToken, MkingTokenPublish } from './MkingToken.js';
...
describe('MkingToken', () => {
    it('success', async () => {
        ...
        let token = new MkingToken(tokenAddress);
        let tokenId = token.deriveTokenId();

        await MkingToken.compile();
        ...
        const zkApp = new MkingTokenPublish(appAddress, tokenId);
        await MkingTokenPublish.compile();
