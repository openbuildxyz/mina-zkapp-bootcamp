...
describe('fund contract', () => {
    ...
    it('首次提取20%', async () => {
        ...
        Local.setBlockchainLength(UInt32.from(15));
        Mina.setActiveInstance(Local);
        ...
        await txn.prove();
        await txn.sign([owner.key]).send();
        Local.setBlockchainLength(UInt32.from(30));
        Mina.setActiveInstance(Local);

        expect(zkApp.account.balance.get()).toEqual(UInt64.from(UInt64.from(100 * 1e9).mul(80).div(100)));
        const now = AccountUpdate.create(owner).account.balance.get()
        expect(now.sub(before)).toEqual(UInt64.from(20 * 1e9))
