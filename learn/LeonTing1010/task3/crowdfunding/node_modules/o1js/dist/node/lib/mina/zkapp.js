...
import { AccountUpdate, Authorization, Body, Events, Permissions, TokenId, ZkappCommand, zkAppProver, ZkappPublicInput, AccountUpdateForest, AccountUpdateLayout, } from './account-update.js';
...
import { HashInput, Poseidon, hashConstant, isHashable, packToFields, } from '../provable/crypto/poseidon.js';
import { UInt32 } from '../provable/int.js';
import * as Mina from './mina.js';
import { assertPreconditionInvariants, cleanPreconditionsCache, } from './precondition.js';
import { analyzeMethod, compileProgram, Empty, getPreviousProofsForProver, sortMethodArguments, VerificationKey, } from '../proof-system/zkprogram.js';
import { Proof } from '../proof-system/proof.js';
import { PublicKey } from '../provable/crypto/signature.js';
import { assertStatePrecondition, cleanStatePrecondition, getLayout, } from './state.js';
import { inAnalyze, inCheckedComputation, inCompile, inProver, } from '../provable/core/provable-context.js';
...
import { getReducer } from './actions/reducer.js';
import { provable } from '../provable/types/provable-derivers.js';
// external API
export { SmartContract, method, declareMethods };
...
function method(target, methodName, descriptor, returnType) {
    ...
    let internalMethodEntry = sortMethodArguments(ZkappClass.name, methodName, paramTypes, undefined, SelfProof);
    // add witness arguments for the publicKey (address) and tokenId
    let methodEntry = sortMethodArguments(ZkappClass.name, methodName, [PublicKey, Field, ...paramTypes], undefined, SelfProof);
...
function wrapMethod(method, ZkappClass, methodIntf) {
    ...
    return async function wrappedMethod(...actualArgs) {
        ...
        if (!insideContract) {
            ...
            try {
                if (inCompile() || inProver() || inAnalyze()) {
                    ...
                    let result;
                    try {
                        result = await assertPromise(method.apply(this, actualArgs.map((a, i) => {
        ...
        let { id, context: innerContext } = SmartContractContext.enter(this, selfAccountUpdate(this, methodName));
        try {
            ...
            let runCalledContract = async () => {
                ...
                let result;
                try {
                    result = await assertPromise(method.apply(this, constantArgs.map(cloneCircuitValue)), noPromiseError);
                }
                finally {
                    memoizationContext.leave(memoId);
                }
                ...
                if (result !== undefined) {
                    ...
                    assert(returnType !== undefined, "Bug: returnType is undefined but the method result isn't.");
    ...
    constructor(address, tokenId) {
        ...
        this.sender = {
            ...
            getUnconstrained() {
                ...
                let sender;
                if (__classPrivateFieldGet(this.self, _SmartContract__senderState, "f")?.transactionId === transactionId) {
                    sender = __classPrivateFieldGet(this.self, _SmartContract__senderState, "f").sender;
                }
                else {
                    sender = Provable.witness(PublicKey, () => Mina.sender());
                    __classPrivateFieldSet(this.self, _SmartContract__senderState, { transactionId, sender }, "f");
                }
...
`);
    ...
    emitEventIf(condition, type, event) {
        ...
        accountUpdate.body.events = Provable.if(condition, Events, newEvents, accountUpdate.body.events);
    }
    /**
...
const ProofAuthorization = {
    setKind({ body, id }, priorAccountUpdates) {
        ...
        let hash = Provable.witness(Field, () => {
            ...
            assert(isProver || priorAccountUpdates !== undefined, 'Called `setKind()` outside the prover without passing in `priorAccountUpdates`.');
...
function diff(transaction, index, prover, input) {
    delete prover.id;
