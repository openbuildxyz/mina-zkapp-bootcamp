...
const balance = (acc: Mina.TestPublicKey | PublicKey) =>
  Mina.getBalance(acc).div(MINA).toJSON();
...
describe('CrowdFunding', () => {
  ...
  const withdraw = async (amount: number) => {
    ...
    console.log(
      'receiver balance: %s, to balance: %s',
      balance(receiverPubKey),
      balance(toAcc)
    );
  };
  ...
  const prepare = async () => {
    ...
    const zkApp = new CrowdFunding(zkAppAccount.toPublicKey());

    const deployTx = await Mina.transaction(deployer, async () => {
      AccountUpdate.fundNewAccount(deployer);
      await zkApp.deploy({
        goal: UInt64.from(100 * MINA),
        endAt: UInt32.from(200),
      });
    });
