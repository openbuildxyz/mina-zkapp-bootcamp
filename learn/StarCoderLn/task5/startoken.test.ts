...
import { StarToken, StarTokenPublish } from './startoken';
...
describe('StarToken', () => {
  it('success', async () => {
    ...
    let token = new StarToken(tokenAddress);
    let tokenId = token.deriveTokenId();

    await StarToken.compile();
    ...
    const zkApp = new StarTokenPublish(appAddress, tokenId);
    await StarTokenPublish.compile();
